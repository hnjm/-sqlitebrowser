...
// +---------------------------+----------+----------+----------+----------+
	...
	virtual void consumeUntil(char_type c)
	{
		...
		{
			char_type la_1 = LA(1);
			if( static_cast<char_type>(std::char_traits<char>::eof()) == la_1 || la_1 == c )
				break;
			consume();
		}
	}
	...
	virtual void consumeUntil(const antlr::BitSet& set)
	{
		...
		{
			char_type la_1 = LA(1);
			if( static_cast<char_type>(std::char_traits<char>::eof()) == la_1 || set.member(la_1) )
				break;
			consume();
		}
	}
	...
	// NOTE: These definitions is probably horribly compiler and platform dependent... Then again, antlr itself is no better when
	// it comes to definining the EOF constant
